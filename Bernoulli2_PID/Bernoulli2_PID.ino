...
#define STD_LOOP_TIME 1500UL   //Estimate time in microsends of each loop
...
void setup() {
  ...
  //Bernoulli const
  b_const = 0;   //Bernoulli constant for failure rate
  //Timer const
  timer_const = 1200000;  //Set the time between target change
  ...
  //SMA const
  ma_period = 3; //Choose the number of samples used in Moving Average, set to 1 if SMA is not used
}
...
void loop() {
  ...
  if (timer >= (timer_const) && timer < (timer_const + STD_LOOP_TIME) && datap == 1) {
    sumError = 0;
    target = 300;
  } else if (timer > (2 * timer_const) - STD_LOOP_TIME-300 && timer < ((2 * timer_const) + STD_LOOP_TIME-300) && datap == 1) {
    sumError = 0;
    target = 800;
  } else if (timer > (3 * timer_const) - STD_LOOP_TIME-300 && timer < ((3 * timer_const) + STD_LOOP_TIME-300) && datap == 1) {
    sumError = 0;
    target = 300;
  } else if (timer > (4 * timer_const) - STD_LOOP_TIME-300 && timer < ((4 * timer_const) + STD_LOOP_TIME-300) && datap == 1) {
    sumError = 0;
    target = 800;
  }else if (timer > (5 * timer_const) - STD_LOOP_TIME-300 && timer < ((5 * timer_const) + STD_LOOP_TIME-300) && datap == 1) {
    sumError = 0;
    target = 300;
  }else if (timer > (6 * timer_const) - STD_LOOP_TIME-300 && timer < ((6 * timer_const) + STD_LOOP_TIME-300) && datap == 1) {
    sumError = 0;
    target = 800;
  ...
  last_pos = pos;
  //end PID controller
  if (datap == 1 && ma_counter < ma_period) {
    //Simple Moving Average SMA
    ma_pos_calc = ma_pos_calc + pos;
    ma_counter = ma_counter + 1;
  }
  if (datap == 1 && ma_counter == ma_period) { //print data if datap=1
    ma_pos = ma_pos_calc / ma_period; //Calc Moving Average
    //Print data to plot
    Serial.print(timer, 0);
    Serial.print("\t");
    Serial.println(ma_pos);
