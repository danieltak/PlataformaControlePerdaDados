...
int ENCODER_READ = analogRead(A0);                       // The encoder digital output must be connected to pin 5 for Timer/Counter1

  float voltage0 = ENCODER_READ * (5.0 / 1023.0);
...
{  
  ...
  case 53:                             // User pressed '5', home
    TCNT1 = 0;                         // set the encoder counting hardware register to 0
                                       // need to reset since we count up for both motor direction moves
    encoderPos = 1000;                 // set a default absolute encoder position; want to ensure non-negative values
...
void motor_backward() 
{
  accel_decel = 0.15;                // the ramp acceleration and deceleration in counts/sample^2
  accel_decel_ramp = 15;             // set the acceleration & deceleration ramp distance in samples
  KP = 1.88;                         // set position gain; my values - yours will very likely be different
  KD = 10.86;                        // set derivative gain; my values - yours will very likely be different
  float accel_decel_dist = 47;    // the expected encoder movement during accel and decel; computed offline via spreadsheet
